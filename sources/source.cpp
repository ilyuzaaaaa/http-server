...
void handle_request(http::request<Body, http::basic_fields<Allocator>>&& req,
                    ...
                    const std::shared_ptr<OutputJSON>& collection) {
...
void suggestion_updater(
    const std::shared_ptr<JsonHandler>& storage,
    const std::shared_ptr<OutputJSON>& suggestions,
    const std::shared_ptr<std::timed_mutex>& mutex) {
...
int Run_server(int argc, char* argv[]) {
  ...
  std::shared_ptr<JsonHandler> storage = std::make_shared<JsonHandler>(
      "/Users/iluzaangirova/http-server/suggestions.json");
  std::shared_ptr<OutputJSON> suggestions =
      std::make_shared<OutputJSON>();
  try {
    if (argc != 3) {
